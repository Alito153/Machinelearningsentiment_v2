...
logger.info("="*80)
logger.info("EVENT STUDY AND LABELING PIPELINE")
logger.info("="*80)
...
logger.info(f"Loaded EURUSD: {len(eurusd_df)} candles, date range: {eurusd_df['timestamp'].min()} to {eurusd_df['timestamp'].max()}")
logger.info(f"Loaded XAUUSD: {len(xauusd_df)} candles, date range: {xauusd_df['timestamp'].min()} to {xauusd_df['timestamp'].max()}")
logger.info(f"Loaded VIX: {len(vix_df)} candles, date range: {vix_df['timestamp'].min()} to {vix_df['timestamp'].max()}")
logger.info(f"Loaded Macro Events: {len(macro_events_df)} events, date range: {macro_events_df['event_timestamp'].min()} to {macro_events_df['event_timestamp'].max()}")
...
def find_time_to_move_pct(prices, open_price, target_pct=0.5):
    if len(prices) < 1:
        return np.nan
    
    target_move = np.abs(np.log(prices[-1] / open_price)) * target_pct
    
    for i, price in enumerate(prices):
        current_move = np.abs(np.log(price / open_price))
...
for idx, event in macro_events_df.iterrows():
    ...
    if len(eurusd_window) > 0:
        eurusd_prices = eurusd_window['close'].values
        ...
        eurusd_15m = eurusd_prices if len(eurusd_prices) >= 16 else eurusd_prices
    ...
    if len(xauusd_window) > 0:
        xauusd_prices = xauusd_window['close'].values
        ...
        xauusd_15m = xauusd_prices if len(xauusd_prices) >= 16 else xauusd_prices
...
vix_regime_df = vix_df[['date', 'close']].rename(columns={'close': 'vix_close'})
...
event_study_df = event_study_df.merge(vix_regime_df, left_on='event_date', right_on='date', how='left')
event_study_df['vix_regime'].fillna(0, inplace=True)
event_study_df['vix_close'].fillna(method='ffill', inplace=True)
